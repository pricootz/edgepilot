...
`DisplayTargetResolver` separates saved display intent from Avalonia's live screen objects. Automatic mode follows the primary available display. Explicit choices are reconciled in order by stable monitor identity, session handle, unique friendly name, then working-area geometry and scaling. On Windows, CCD (`QueryDisplayConfig`) supplies EDID names, stable device paths and OS-reported `targetAvailable`; a low-frequency health check catches availability changes that do not produce an Avalonia screen event. If no usable match exists, placement temporarily uses an available display while preserving the saved target for a later reconnect. Some monitors keep their logical path active when only their physical power button is used, so Settings and the Windows tray expose an explicit move-to-current-display recovery path. Screen-collection change bursts remain debounced before relocation and Settings refresh.
