...
`DisplayTargetResolver` separates saved display intent from Avalonia's live screen objects. Automatic mode follows the primary available display. Explicit choices are reconciled in order by stable monitor identity, session handle, unique friendly name, then working-area geometry and scaling. On Windows, CCD (`QueryDisplayConfig`) supplies EDID names, stable device paths and OS-reported `targetAvailable`; device identities also provide a model-code label when the friendly name is absent. A low-frequency health check catches availability changes that do not produce an Avalonia screen event. If no usable match exists, placement temporarily uses an available display while preserving the saved target for a later reconnect. Some monitors keep their logical path active when only their physical power button is used, so the Windows tray exposes every display as an explicit recovery destination. Recovery bypasses cursor ambiguity, applies the target position to the HWND, verifies its `HMONITOR`, and briefly reveals the panel. Screen-collection change bursts remain debounced before relocation and Settings refresh.
